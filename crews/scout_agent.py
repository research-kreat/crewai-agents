...
class ScoutAgent:
    ...
    def _extract_keywords(self, prompt):
        """Generate keywords using LLM based on the prompt."""
        task = Task(
            description=f"Extract keywords Only from the following user's prompt:\n{prompt}",
            expected_output="Array of keywords in ['...','...','...',...]",
            agent=self.agent,
        )
        ...
        try:
            ...
            # Assuming the result is a string representation of a list, e.g. '["keyword1", "keyword2"]'
            # Convert the result string to an actual Python list
            keywords_str = str(result).strip()
            if keywords_str:
                keywords = eval(keywords_str)
                if not isinstance(keywords, list):
                    raise ValueError("Keywords are not in a valid list format.")
                return keywords
            else:
                raise ValueError("No keywords generated by the model.")
